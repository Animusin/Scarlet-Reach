	...
	if (registered_z != new_z)
		...
		if (client)
			if (new_z)
				...
				for (var/I in length_char(SSidlenpcpool.idle_mobs_by_zlevel[new_z]) to 1 step -1) //Backwards loop because we're removing (guarantees optimal rather than worst-case performance), it's fine to use .len here but doesn't compile on 511
