...
/world/Error(exception/E, datum/e_src)
	...
	if(copytext_char(E.name,1,32) == "Maximum recursion level reached")
	...
	else if(copytext_char(E.name,1,18) == "Out of resources!")
	...
	if (islist(stack_trace_storage))
		for (var/line in splittext_char(E.desc, "\n"))
			if (text2ascii_char(line) != 32)
