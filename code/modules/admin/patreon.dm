...
/proc/load_patreons()
	...
	for(var/line in csvlist)
		if(findtext_char(line, "ROGUETOWN SILVER"))
			if(findtext_char(line, "Active patron"))
				var/index = findtext_char(line, ",")
				var/indexs = findtext_char(line, ",",index+1)
				var/player_email = copytext_char(line,index+1,indexs)
//				player_email = sanitize_simple(player_email,list("@"="AT","."="DOT"))
	...
	for(var/line in csvlist)
		if(findtext_char(line, "ROGUETOWN GOLD"))
			if(findtext_char(line, "Active patron"))
				var/index = findtext_char(line, ",")
				var/indexs = findtext_char(line, ",",index+1)
				var/player_email = copytext_char(line,index+1,indexs)
//				player_email = sanitize_simple(player_email,list("@"="AT","."="DOT"))
	...
	for(var/line in csvlist)
		if(findtext_char(line, "ROGUETOWN MYTHRIL"))
			if(findtext_char(line, "Active patron"))
				var/index = findtext_char(line, ",")
				var/indexs = findtext_char(line, ",",index+1)
				var/player_email = copytext_char(line,index+1,indexs)
//				player_email = sanitize_simple(player_email,list("@"="AT","."="DOT"))
	...
	for(var/line in csvlist)
		if(findtext_char(line, "ROGUETOWN MERCHANT"))
			if(findtext_char(line, "Active patron"))
				var/index = findtext_char(line, ",")
				var/indexs = findtext_char(line, ",",index+1)
				var/player_email = copytext_char(line,index+1,indexs)
//				player_email = sanitize_simple(player_email,list("@"="AT","."="DOT"))
	...
	for(var/line in csvlist)
		if(findtext_char(line, "ROGUETOWN LORD"))
			if(findtext_char(line, "Active patron"))
				var/index = findtext_char(line, ",")
				var/indexs = findtext_char(line, ",",index+1)
				var/player_email = copytext_char(line,index+1,indexs)
...
/proc/get_patreon_manual(ckey)
	...
	if(fexists(csv_file))
		csvlist = world.file2list(csv_file)
	for(var/line in csvlist)
		if(findtext_char(line, the_email))
			if(findtext_char(line, "Active patron"))
				if(findtext_char(line, "ROGUETOWN SILVER"))
					return 1
				if(findtext_char(line, "ROGUETOWN GOLD"))
...
/mob/dead/new_player/proc/register_patreon()
	...
	if(!email)
		return
	if(!patreon_lookup(name) || !patreon_lookup(email) || !findtext_char(email, "@"))
