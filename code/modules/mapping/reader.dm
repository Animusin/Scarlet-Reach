	...
	for(var/model_key in grid_models)
		...
		while(dpos != 0)
			...
			if(variables_start)//if there's any variable
				full_def = copytext_char(full_def,variables_start+1,length_char(full_def))//removing the last '}'
		...
		if(no_changeturf \
			...
			&& length_char(members_attributes[1]) == 0 \
			&& length_char(members_attributes[2]) == 0 \
...
/datum/parsed_map/proc/parse_constant(text)
	...
	if(copytext_char(text,1,6) == "list(")
		return readlist(copytext_char(text,6,length_char(text)))
	...
	// file
	if(copytext_char(text,1,2) == "'")
		return file(copytext_char(text,2,length_char(text)))
