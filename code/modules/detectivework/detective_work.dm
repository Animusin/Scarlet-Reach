...
/atom/proc/add_fibers(mob/living/carbon/human/M)
	...
	if(M.gloves && istype(M.gloves, /obj/item/clothing))
		...
		old = length_char(G.return_blood_DNA())
		if(G.transfer_blood > 1) //bloodied gloves transfer blood to touched objects
			if(add_blood_DNA(G.return_blood_DNA()) && length_char(G.return_blood_DNA()) > old) //only reduces the bloodiness of our gloves if the item wasn't already bloody
				G.transfer_blood--
	else if(M.bloody_hands > 1)
		old = length_char(M.return_blood_DNA())
		if(add_blood_DNA(M.return_blood_DNA()) && length_char(M.return_blood_DNA()) > old)
			M.bloody_hands--
	var/datum/component/forensics/D = AddComponent(/datum/component/forensics)
	. = D.add_fibers(M)
